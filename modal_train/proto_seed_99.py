"""ProtoSSM(d=320) 4-fold, seed=99 — for ensemble diversity."""
import modal
app = modal.App("p320-seed-99")
...
def train():
    ...
    rng=np.random.default_rng(99); fi=np.arange(nf); rng.shuffle(fi); kfs=KFold(4,shuffle=False)
    ...
    for fid,(tp,vp) in enumerate(kfs.split(fi),1):
        ...
        sp=f"/vol/checkpoints/moe_protossm320_seed99_fold{fid}.pt"
    ...
    vol.commit(); print(f"DONE seed=99: {aucs} mean={sum(aucs)/4:.4f}"); return aucs
