import modal
app = modal.App("birdclef-protossm256")
...
@app.function(image=image, gpu="A100", volumes={"/vol":vol}, timeout=3600)
def train(d_model=256, seed=2024, n_epochs=100):
    ...
    fhl = (Y.sum(1)>0).reshape(len(meta)//NW, NW).any(1)
    kf = np.where(fhl)[0]
    ki = np.concatenate([np.arange(f*NW,(f+1)*NW) for f in kf])
    emb,sc,Y = emb[ki],sc[ki],Y[ki]; mk = meta.iloc[ki].reset_index(drop=True)
    nf = len(emb)//NW
    su = sorted(mk["site"].dropna().astype(str).unique())
    s2i = {s:i+1 for i,s in enumerate(su)}; NCAP = max(32, len(su)+2)
    ...
    class _SSM(nn.Module):
        def __init__(self,d,ds=24):
            ...
            self.ip=nn.Linear(d,2*d,bias=False); self.c=nn.Conv1d(d,d,4,padding=3,groups=d)
            self.dt=nn.Linear(d,d,bias=True)
            A=torch.arange(1,ds+1,dtype=torch.float32).unsqueeze(0).expand(d,-1)
        ...
        def forward(self,x):
            ...
            dt=F.softplus(self.dt(xc)); A=-torch.exp(self.A)
            B=self.B(xc); C=self.C(xc); h=torch.zeros(B_sz,D,self.ds,device=x.device); ys=[]
            for t in range(T):
    ...
    class P(nn.Module):
        def __init__(self,d,nc=NC,ns=NCAP,nl=3,ds=24):
            super().__init__()
            ...
            self.f=nn.ModuleList([_SSM(d,ds) for _ in range(nl)]); self.b=nn.ModuleList([_SSM(d,ds) for _ in range(nl)])
        ...
        def forward(self,e,tl,si,hi):
            h=self.proj(e)+self.pe[:,:e.size(1),:]
            m=self.mp(torch.cat([self.se(si),self.he(hi)],-1)); h=h+m[:,None,:]
            for f,b,mm,nn_ in zip(self.f,self.b,self.m,self.n):
                r=h; hf=f(h); hb=b(h.flip(1)).flip(1); h=nn_(r+self.dr(mm(torch.cat([hf,hb],-1))))
            hn=F.normalize(h,-1); pn=F.normalize(self.pt,-1)
            si_=torch.matmul(hn,pn.T)*F.softplus(self.tp)+self.cb[None,None,:]
            al=torch.sigmoid(self.fa)[None,None,:]; return al*si_+(1-al)*tl
    ef = emb.reshape(nf,NW,-1); lf = sc.reshape(nf,NW,-1); yf = Y.reshape(nf,NW,-1)
    rng = np.random.default_rng(seed); fi = np.arange(nf); rng.shuffle(fi)
    kfs = KFold(4, shuffle=False)
    os.makedirs("/vol/checkpoints", exist_ok=True)
    aucs = []
    for fid,(tp,vp) in enumerate(kfs.split(fi),1):
        ti=fi[tp]; vi=fi[vp]; m = P(d_model).cuda()
        with torch.no_grad():
            eft = torch.from_numpy(ef[ti].reshape(-1,1536)).float().cuda()
            yft = torch.from_numpy(yf[ti].reshape(-1,NC)).float().cuda()
            h0 = m.proj(eft)
            for c in range(NC):
                mm = yft[:,c]>0.5
        ...
        sch=torch.optim.lr_scheduler.OneCycleLR(opt,max_lr=1e-3,epochs=n_epochs,steps_per_epoch=1,pct_start=0.1,anneal_strategy="cos")
        sw=torch.optim.swa_utils.AveragedModel(m); ss=int(n_epochs*0.65); sws=torch.optim.swa_utils.SWALR(opt,swa_lr=4e-4)
        best=0
        for ep in range(n_epochs):
            ...
            if (ep+1)%25==0 or ep==n_epochs-1:
                m.eval()
                with torch.no_grad():
                    ov=m(ev,lv,sv,hv).cpu().numpy()
                pv=1/(1+np.exp(-np.clip(ov,-30,30))); pvf=pv.reshape(-1,NC); yvf=yv.reshape(-1,NC)
                v=yvf.sum(0)>0
        ...
        sp=f"/vol/checkpoints/moe_protossm{d_model}_seed{seed}_fold{fid}.pt"
        torch.save(sw.module.state_dict() if hasattr(sw,'module') else sw.state_dict(), sp)
        print(f"fold{fid} best={best:.4f} saved {sp}", flush=True)
        aucs.append(best)
    vol.commit(); print(f"DONE: {aucs} mean={sum(aucs)/len(aucs):.4f}"); return aucs
