...
Student spec (match public SED I/O for drop-in replacement):
  ...
  Backbone: tf_efficientnetv2_s.in21k (~21M params, 2025-winners proven; diversity partner)
...
app = modal.App("birdclef-sed-student-effv2s")
...
out_vol = modal.Volume.from_name("birdclef-sed-student-effv2s", create_if_missing=True)
...
def train(
    ...
    # ── Student model: EffV2-S (in21k pretrained) — standard SED head
    class SEDStudent(nn.Module):
        def __init__(self, backbone_name="tf_efficientnetv2_s.in21k", n_classes=NC):
            super().__init__()
