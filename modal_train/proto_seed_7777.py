"""ProtoSSM(d=320) 4-fold, seed=7777 — for ensemble diversity."""
import modal
app = modal.App("p320-seed-7777")
...
def train():
    ...
    rng=np.random.default_rng(7777); fi=np.arange(nf); rng.shuffle(fi); kfs=KFold(4,shuffle=False)
    ...
    for fid,(tp,vp) in enumerate(kfs.split(fi),1):
        ...
        sp=f"/vol/checkpoints/moe_protossm320_seed7777_fold{fid}.pt"
    ...
    vol.commit(); print(f"DONE seed=7777: {aucs} mean={sum(aucs)/4:.4f}"); return aucs
